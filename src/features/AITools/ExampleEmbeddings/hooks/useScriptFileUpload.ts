...
import { openScriptFileDialog, readScriptFile } from '../../api'
import { useState } from 'react'

import type { FileValidation, UploadError } from '../../types'
...
export function useScriptFileUpload(): UseScriptFileUploadReturn {
  ...
  const selectFile = async (): Promise<string | null> => {
    try {
      return await openScriptFileDialog()
    } catch (err) {
      setError({
        type: 'network_error',
  ...
  const readFileContent = async (path: string): Promise<string> => {
    ...
    try {
      const content = await readScriptFile(path)
      setIsReading(false)
      return content
    } catch (err) {
      setIsReading(false)
