/**
 * Kavanagh Settings Section (issue #180)
 *
 ...
 */
import { useKavanaghAvailability } from '@features/Kavanagh'
import { KAVANAGH_THRESHOLDS, validateMatchConfidence } from '@shared/constants'
...
const KavanaghSection: React.FC<KavanaghSectionProps> = ({
  apiKeys,
  ...
  const [referenceFolder, setReferenceFolder] = React.useState<string | null>(
    apiKeys.kavanaghReferenceFolder ?? null
  )
  ...
  const [matchThreshold, setMatchThreshold] = React.useState<string>(
    apiKeys.kavanaghMatchThreshold !== undefined
      ? String(apiKeys.kavanaghMatchThreshold)
      : ''
  )
  const thresholdProblem = validateMatchConfidence(matchThreshold)

  const { reason, available, pending } = useKavanaghAvailability()
  ...
  const { data: folderPresent } = useQuery({
    queryKey: queryKeys.kavanagh.referenceFolderPresent(referenceFolder),
    queryFn: async () => {
  ...
  return (
    ...
    >
      <div className="border-b pb-2">
        <h3 className="text-foreground text-lg font-semibold">Kavanagh</h3>
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={() =>
              save(
                { kavanaghReferenceFolder: referenceFolder ?? undefined },
                'reference folder'
              )
            }
      ...
      */}
      <details>
        ...
        <div className="mt-3">
          <label
            htmlFor="kavanagh-match-threshold"
            className="mb-2 block text-sm font-medium"
          >
            Watermark match confidence
          </label>
          <p className="text-muted-foreground mb-2 text-xs">
            Leave empty to use the calibrated default of{' '}
            {KAVANAGH_THRESHOLDS.matchConfidence}. A genuine match measures about 0.98 and
            a corner with no watermark about 0.01, so the default sits in a wide empty
            band. Any run using an override says so in its report.
          </p>
          <div className="flex items-center gap-2">
            <Input
              id="kavanagh-match-threshold"
              type="text"
              inputMode="decimal"
              ...
              aria-describedby={
                thresholdProblem ? 'kavanagh-match-threshold-error' : undefined
              }
              className="max-w-32"
            />
            <Button
          ...
          {thresholdProblem && (
            <p
              id="kavanagh-match-threshold-error"
              role="alert"
...
export default KavanaghSection
