/**
 * Kavanagh settings section (issue #180, B14)
 *
...
vi.mock('@features/Kavanagh', () => ({ useKavanaghAvailability: vi.fn() }))
...
beforeEach(async () => {
  ...
  const qc = await import('@features/Kavanagh')
  vi.mocked(qc.useKavanaghAvailability).mockReturnValue(READY_AVAILABILITY as never)
})

describe('KavanaghSection', () => {
  ...
  it('B14.2 saves the reference folder without dropping existing keys', async () => {
    ...
    renderSection({ ...EXISTING_KEYS, kavanaghReferenceFolder: FOLDER })
    ...
    expect(api.saveSettingsApiKeys).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        trello: 'trello-key',
        kavanaghReferenceFolder: FOLDER
      })
    )
  })
  ...
  it('B14.3 refuses to save while the settings file is unreadable', () => {
    renderSection({ ...EXISTING_KEYS, kavanaghReferenceFolder: FOLDER }, true)
  ...
  it('B14.4 warns when the configured reference folder cannot be read', async () => {
    ...
    renderSection({ ...EXISTING_KEYS, kavanaghReferenceFolder: '/Volumes/Gone/refs' })
  ...
  it('B14.5 surfaces the live ffmpeg detection reason', async () => {
    const qc = await import('@features/Kavanagh')
    vi.mocked(qc.useKavanaghAvailability).mockReturnValue({
      ...READY_AVAILABILITY,
      available: false,
  ...
  it('B13.2 saves a valid match confidence override', async () => {
    ...
    await waitFor(() => expect(api.saveSettingsApiKeys).toHaveBeenCalled())
    expect(api.saveSettingsApiKeys).toHaveBeenCalledWith(
      expect.objectContaining({ kavanaghMatchThreshold: 0.35 })
    )
  })
  ...
  it('B13.1 clears the override back to the default when the field is emptied', async () => {
    const user = userEvent.setup()
    renderSection({ ...EXISTING_KEYS, kavanaghMatchThreshold: 0.35 })
    ...
    expect(api.saveSettingsApiKeys).toHaveBeenCalledWith(
      expect.objectContaining({ kavanaghMatchThreshold: undefined })
    )
  })
})
