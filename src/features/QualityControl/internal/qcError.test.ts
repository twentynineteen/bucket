...
import { asQcError, isCancellation } from './qcError'

describe('asQcError', () => {
  it('keeps a tagged error from the backend as it is', () => {
    const error = asQcError({
      kind: 'ffmpeg',
  ...
  it('keeps an unrecognised shape readable rather than showing an object', () => {
    ...
    const error = asQcError(new Error('the webview lost the connection'))
  ...
  it('does not mistake an arbitrary object for a tagged error', () => {
    const error = asQcError({ kind: 'somethingElse', message: 'nope' })
