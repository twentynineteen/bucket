...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        // NB: Tailwind v4 arbitrary CSS-variable values need var() - the bare
        // `max-h-[--radix-...]` shadcn v3 form silently produces no rule under
        // v4, leaving the menu uncapped so a list taller than the window
        // overflows off-screen with no scroll (#274).
    ...
    >
      ...
      <SelectPrimitive.Viewport
        className={cn(
          'p-1',
          // Deliberately NOT setting `h-[var(--radix-select-trigger-height)]`
          // here: pinning the viewport to the trigger's height defeats the
          // content's `max-h-[--radix-select-content-available-height]` +
          // overflow, so a list taller than the window overflows off-screen
          // with no way to scroll (#274). Left unset, the viewport grows to its
          // content, the content caps at the available height, and the wheel
          // scrolls it.
