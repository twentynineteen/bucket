...
describe('@shared/lib barrel contract', () => {
  ...
  describe('behavior: query-keys factory', () => {
    test('produces expected key structures for all domains', () => {
      ...
      expect(queryKeys.os.username()).toEqual(['os', 'username'])
      expect(queryKeys.navigation.breadcrumb()).toEqual(['navigation', 'breadcrumb'])
    ...
    test('validateQueryKey validates structure', () => {
      ...
      expect(validateQueryKey(['os', 'username'])).toBe(true)
      expect(validateQueryKey(['navigation', 'breadcrumb'])).toBe(true)
      expect(validateQueryKey(['user', 'username'])).toBe(false)
