...
impl KavanaghError {
    ...
    pub fn message(&self) -> &str {
        match self {
            KavanaghError::Busy { message }
            | KavanaghError::Cancelled { message }
            | KavanaghError::Unavailable { message }
            | KavanaghError::Probe { message }
            | KavanaghError::Ffmpeg { message, .. }
            | KavanaghError::ReferencePool { message }
            | KavanaghError::Threshold { message }
            | KavanaghError::Io { message } => message,
        }
    }
}
...
mod tests {
    ...
    fn serialises_with_the_tag_the_frontend_matches_on() {
        ...
        // `src/features/Kavanagh/types.ts`. The two must stay in step.
        let json = serde_json::to_string(&KavanaghError::Ffmpeg {
    ...
    fn every_variant_carries_a_message_the_ui_can_render() {
        let errors = [
            ...
            KavanaghError::Cancelled {
                message: "cancelled".to_string(),
            },
            KavanaghError::Unavailable {
                message: "unavailable".to_string(),
            },
            KavanaghError::Probe {
                message: "probe".to_string(),
            },
            KavanaghError::Ffmpeg {
                message: "ffmpeg".to_string(),
                stderr: String::new(),
            },
            KavanaghError::ReferencePool {
                message: "pool".to_string(),
            },
            KavanaghError::Threshold {
