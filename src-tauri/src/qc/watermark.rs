...
pub fn analyse(
    ...
    let (threshold, threshold_is_default) = resolve_match_confidence(request.match_threshold)
        .map_err(|message| QcError::Threshold { message })?;
...
fn probe_video(
    ...
    if !run.exit_ok {
        ...
        return Err(QcError::Ffmpeg {
            message: format!("ffprobe could not read {}.", request.video_path),
            stderr: run.stderr_tail,
        });
    }

    parse_probe_output(&String::from_utf8_lossy(&stdout)).map_err(|problem| QcError::Probe {
...
fn prepare_references(
    ...
    probe: &VideoProbe,
    cancel: &watch::Receiver<bool>,
) -> Result<PreparedReferences, QcError> {
    struct Measured {
        path: String,
        corner: Corner,
        region: CropRegion,
...
fn run_error(error: RunError) -> QcError {
    match error {
        RunError::Cancelled => QcError::Cancelled {
            message: "The quality control run was cancelled.".to_string(),
        },
        RunError::Spawn(message) => QcError::Unavailable { message },
        RunError::Io(message) => QcError::Io { message },
    }
}
