...
pub fn save_evidence(
    ...
    for item in items {
        ...
        fs::write(&target, &item.jpeg).map_err(|e| QcError::Io {
...
mod tests {
    use super::*;
    use crate::qc::watermark::Thumbnail;
    ...
    fn b10_2_writes_every_thumbnail_into_the_chosen_folder() {
        ...
        assert!(
            written[0].ends_with("qc-render-watermark-missing-12.0s.jpg"),
            "got {}",
            written[0]
        );
    }
    ...
    fn b10_3_never_overwrites_a_file_already_in_the_folder() {
        let dir = tempdir().unwrap();
        let existing = dir.path().join("qc-render-watermark-missing-12.0s.jpg");
    ...
    fn reports_a_folder_that_cannot_be_written_to() {
        ...
        assert!(matches!(result, Err(QcError::Io { .. })));
    }
    ...
    fn strips_characters_a_filename_cannot_carry() {
        ...
        assert_eq!(
            evidence_stem("qc", "watermark missing 04:12/04:31"),
            "qc-watermark-missing-04-12-04-31"
        );
    }
}
